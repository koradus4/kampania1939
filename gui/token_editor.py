...
class TokenEditor:
    ...
    def build_controls(self):
        ...
        unit_types = [
            ...
            ("Zaopatrzenie (Z)", "Z", tk.DISABLED),  # Ustawiono jako nieaktywne
        ...
        for label_text, var, fg_color in [("Punkty Ruchu:", self.movement_points, "white"),
                              ("Zasięg Ataku:", self.attack_range, "white"),
                              ("Wartość Ataku:", self.attack_value, "white"),
                              ("Wartość Bojowa:", self.combat_value, "white"),
                              ("Koszt Utrzymania:", self.unit_maintenance, "red"),  # Zmieniono etykietę i kolor
                              ("Wartość Zakupu:", self.purchase_value, "white"),
                              ("Zasięg Widzenia:", self.sight_range, "white")]:  # Dodane nowe pole
    ...
    def update_numeric_fields(self):
        defaults = {
            ...
            "unit_maintenance": {
                "Pluton": {
                    "P": "2", "K": "3",
                    "TC": "5", "TŚ": "4", "TL": "3", "TS": "2",  # Koszt utrzymania dla plutonu
                    "AC": "3", "AL": "3", "AP": "3",
                    "Z": "2", "D": "0", "G": "0"
                },
                "Kompania": {
                    "P": "4", "K": "6",
                    "TC": "10", "TŚ": "8", "TL": "6", "TS": "5",  # Koszt utrzymania dla kompanii
                    "AC": "6", "AL": "6", "AP": "6",
                    "Z": "4", "D": "0", "G": "0"
                },
                "Batalion": {
                    "P": "8", "K": "9",
                    "TC": "15", "TŚ": "12", "TL": "10", "TS": "8",  # Koszt utrzymania dla batalionu
                    "AC": "12", "AL": "10", "AP": "9",
                    "Z": "6", "D": "0", "G": "0"
                }
            },
    ...
    def create_token_image(self, custom_size=None, token_name=None):
        ...
        # Ulepszona funkcja wyszukiwania czcionki
        def get_font(font_name, size):
            try:
                # Próba załadowania czcionki z systemowych czcionek
                return ImageFont.truetype(font_name, size)
            except IOError:
                try:
                    # Próba załadowania czcionki z katalogu aplikacji
                    font_path = os.path.join(get_application_path(), f"{font_name}")
                    return ImageFont.truetype(font_path, size)
                except Exception:
                    # Używanie domyślnej czcionki jako ostateczność
                    return ImageFont.load_default()
                    
        font_small = get_font("arial.ttf", small_font_size)
    
        # Przygotowanie tekstu – trzy wiersze
        row_top_text = f"{self.combat_value.get()}-{self.unit_maintenance.get()}-{self.purchase_value.get()}"
    ...
    def save_token(self):
        ...
        # Tworzenie podkatalogu dla tokena
        token_directory = os.path.join(self.save_directory, token_name)
        ...
        # Lista plików do śledzenia
        copied_files = [f"{token_name}.png"]
        
        # Kopiowanie plików dźwiękowych do katalogu tokena
        sound_paths = {
            "sound_attack": self.sound_attack,
            "sound_move": self.sound_move,
            "sound_destroy": self.sound_destroy
        }
        
        copied_sound_paths = {}
        for sound_type, orig_path in sound_paths.items():
            if orig_path and os.path.exists(orig_path):
                # Stwórz nazwę pliku docelowego bazując na nazwie tokena i typie dźwięku
                file_ext = os.path.splitext(orig_path)[1]
                sound_name = sound_type.split('_')[1]
                filename = f"{token_name}_{sound_name}{file_ext}"
                dest_path = os.path.join(token_directory, filename)
                
                # Kopiuj plik dźwiękowy
                shutil.copy2(orig_path, dest_path)
                copied_sound_paths[sound_type] = filename  # Zapisz tylko nazwę pliku zamiast pełnej ścieżki
                copied_files.append(filename)
            else:
                copied_sound_paths[sound_type] = None
        
        # Kopiowanie tła, jeśli używane
        bg_path = None
        if self.custom_bg_path and os.path.exists(self.custom_bg_path):
            bg_ext = os.path.splitext(self.custom_bg_path)[1]
            bg_filename = f"{token_name}_background{bg_ext}"
            bg_dest_path = os.path.join(token_directory, bg_filename)
            shutil.copy2(self.custom_bg_path, bg_dest_path)
            bg_path = bg_filename  # Zapisz tylko nazwę pliku
            copied_files.append(bg_filename)
        ...
        token_data = {
            ...
            "png_file": f"{token_name}.png",  # Tylko nazwa pliku zamiast pełnej ścieżki
            "width": final_size,
            "height": final_size,
            ...
            "background_file": bg_path,  # Nazwa pliku tła
            "sound_attack_file": copied_sound_paths["sound_attack"],  # Nazwy plików dźwięków
            "sound_move_file": copied_sound_paths["sound_move"],
            "sound_destroy_file": copied_sound_paths["sound_destroy"]
        }
        
        # Zapisz token_data.json w podkatalogu tokena
        token_json_path = os.path.join(token_directory, "token_data.json")
        with open(token_json_path, "w", encoding="utf-8") as f:
            json.dump(token_data, f, indent=2, ensure_ascii=False)
        
        copied_files.append("token_data.json")
        
        # Zapisz informację o tokenie również w głównym pliku indeksu
        ...
        # Dodaj podstawowe informacje o tokenie do indeksu
        index_data[token_name] = {
            "directory": token_name,
            "nation": self.nation.get(),
            "unit_type": self.unit_type.get(),
            "unit_size": self.unit_size.get()
        }
        ...
        # Przygotowanie tekstu informacyjnego
        info_text = f"Żeton '{token_name}' został zapisany.\n\nPliki zapisane w podkatalogu: {token_directory}:"
        
        # Dodaj listę wszystkich skopiowanych plików
        for copied_file in copied_files:
            info_text += f"\n- {copied_file}"
        
        # Dodaj informację o pliku JSON
        info_text += f"\n- token_data.json (metadane żetonów w katalogu głównym)"
        
        messagebox.showinfo("Zapisano", info_text)
        
        # Po zapisaniu żetonu aktualizuj stan przycisków
